...
def generate_torchgeom_dataset(data, sig=0.0):
    ...
    for sim_ind in range(n_sims):
        ...
        tg_data = Data(
            ...
            y=torch.Tensor(data['u'][sim_ind][:, use_nodes]),
...
def plot_fields(t, coords, fields, delay=0.1, save_path=None):
    ...
    #block=False
    fig.show()
   # fig.savefig(save_path+"TEMP.png")
    ...
    for j, tj in enumerate(t):
        ...
        if save_path is not None:
            plt.savefig(save_path+"{:s}_{:d}.png".format(key, j))
            print("saving"+save_path+"{:s}_{:d}.png".format(key, j) )
